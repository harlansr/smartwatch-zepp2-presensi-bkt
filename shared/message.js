...
class Session extends EventBus {
  ...
  addChunk(payload) {
    ...
    if (payload.payloadLength !== payload.payload.byteLength) {
      DEBUG_2 && logger.error('receive chunk data length error, expect %d but %d', payload.payloadLength, payload.payload.byteLength)
  ...
  checkIfReceiveAllChunks() {
    ...
    if (this.finishChunk.totalLength !== this.finishChunk.payloadLength) {
      DEBUG_2 && logger.error('receive full data length error, expect %d but %d', this.finishChunk.payloadLength, this.finishChunk.payload.byteLength)
...
export class MessageBuilder extends EventBus {
  ...
  disConnect(cb) {
    DEBUG_2 && logger.debug('app ble disconnect')
    this.sendClose()
    this.off('message')
  ...
  onFragmentData(bin) {
    ...
    if (data.flag === MessageFlag.App && data.type === MessageType.Shake) {
      ...
      DEBUG_2 && logger.debug('appSidePort=>', data.port2)
      ...
      DEBUG_2 && logger.error('error appSidePort=>%d data=>%j', this.appSidePort, data)
    }
  }
